...
現在はProtocol BufferのソースコードをCI上では毎回ビルドするなど、余計な時間がかかっている。
Bazelにはremote cacheという機能が存在し、ビルドの状態をキャッシュしておくことで次回以降のビルド時間を短縮できる。
https://bazel.build/remote/caching

このremote cacheは開発者全員が参照可能で、一度ビルドされたものを全員が共有できる。

## GitHub Actionsのキャッシュの問題点

GitHub Actionsにもキャッシュ機能があるため、これを使ってCI上でのビルド時間を短縮することは最近のベストプラクティスの一つである。
一方、GitHub ActionsのキャッシュをBazelと組み合わせる場合、以下の問題点がある。
...
- GitHub Actionsのキャッシュに対して開発者の環境からアクセスすることが難しい
  - Bazelはその強力なキャッシュ機能から、全てを再ビルドすることなくビルド結果を流用できることが強みである
  - しかし、GitHub ActionsのキャッシュはGitHub Actions上でのみ利用できるため、開発者のローカル環境からは利用できない

これらの問題点を解決するために、Bazelを採用する場合はリモートキャッシュを使うことが推奨される。

## Use Google Cloud Storage

最も簡単な方法は、Google Cloud Storage（GCS）を使う方法である。BazelはGCSをネイティブにサポートしており、単純にバケットを作ってクレデンシャルを渡すだけでリモートキャッシュとして利用できる。
https://bazel.build/versions/7.4.0/remote/caching#cloud-storage

GCSのバケットを作成し、`.bazelrc`に以下の内容を追記する。
...
これでCI上でのビルドで作成されたキャッシュがGCSにアップロードされ、開発者全員もそれを参照できビルドが高速化する。
またCI上からもこのキャッシュが参照されるため、CI上でのビルドも高速化される。

> [!TIP]
> GCSを使う場合、リモートキャッシュの肥大化を防ぐためライフサイクルポリシーを設定した方がよい。
> これにより古いキャッシュが自動的に削除されるため、ストレージコストを抑えることができる。
> Ref: https://cloud.google.com/storage/docs/managing-lifecycles?hl=ja

> [!TIP]
> GitHub Actionsと組み合わせる場合、バケットのリージョンを考慮しなければならない。
> [GitHub Hosted runner自体はUSのリージョンにある](https://github.com/orgs/community/discussions/24969)ため、バケットもUSリージョンに作成することで通信速度を向上させることができる。
> しかし、日本の開発者がUSリージョンのバケットにアクセスする際には通信速度が遅くなってしまう。

## Use AWS S3 / Azure Blob Storage / local storage

AWS S3（およびその互換APIを提供するストレージサービス）やAzure Blob Storageはbazelではネイティブにはサポートされていない。しかし、 [bazel-remote](https://github.com/buchgr/bazel-remote)を使うことで、リモートキャッシュとして利用できる。また、サーバーのローカルファイルシステムをリモートキャッシュとして提供することも可能である。

> [!WARNING]
> 2024/10現在、bazel-remoteはキャッシュの一貫性を保つため、1インスタンスのみ存在することを想定している。そのため高可用性を確保する構成にすることが難しい。
> https://github.com/buchgr/bazel-remote/issues/483
> OSごとに異なるインスタンスを用意して接続先を切り替えるなど、負荷分散の方法は単純にレプリカを増やすなどに留まらない。
> また、最大のパフォーマンスのためには高速なSSDなどのローカルストレージを利用することが望ましいとメンテナは述べている。
> https://github.com/buchgr/bazel-remote/issues/786#issuecomment-2418009617

> [!NOTE]
> BazelのリモートキャッシュはProtocol Bufferで定義されたgRPCのAPIから構成され、bazel-remoteはそれらのAPIの一部を実装しているOSSである。
> https://github.com/bazelbuild/remote-apis/blob/main/build/bazel/remote/execution/v2/remote_execution.proto
> 自前でこのようなAPIを実装することで、独自のリモートキャッシュを構築することも可能である。

bazel-remoteのコンテナイメージが提供されているため、これを使うことで簡単にリモートキャッシュを構築できる。
今回はどの家庭にもあるKubernetesクラスタにbazel-remoteをデプロイする。

```bash
# 今回はbazelremoteという名前空間を使う
NS=bazelremote
kubectl create ns ${NS}
```

S3バックエンドを使うためのクレデンシャルを用意する。
...
bazel-remoteをStatefulSetとしてデプロイするマニフェストを作成する。
...
PodがRunningになったら、kubectlでport-forwardを使ってローカルからアクセスできるようにする。
...
> [!NOTE]
> 実際に運用する場合にはIngressリソースや `type: LoadBalancer` なServiceを使って外部公開したり、[Actions Runner Controller](https://docs.github.com/en/actions/hosting-your-own-runners/managing-self-hosted-runners-with-actions-runner-controller/about-actions-runner-controller)などを使ってクラスタ内にGitHub Actions Runnerを建てるなどの方法がある。
> 外部公開する場合には、BASIC認証やmTLSを使ってセキュリティを強化することが推奨される。

まずはキャッシュを削除してビルドの流れを二回ほど実施してみる。

> [!WARNING]
> あくまでこの手順はデモ用であり、実際の性能向上を保証するものではない。
> この手順は筆者の自宅KubernetesクラスタにあるMinioを使っているため、AWS S3を使う場合とは傾向が異なる可能性がある。
...
いずれも約100秒程度かかった。では、キャッシュへの書き込みおよび参照を有効にしてみる。
...
一方、2回目のビルドは以下の様に7秒で完了した。これがリモートキャッシュによる効果である。
このリモートキャッシュは開発者全員が参照するようにすれば、開発者全員がビルド結果を共有できるため、ビルド時間の短縮に繋がる。

## Conclusion

Bazelにはビルドした結果をキャッシュして外部に保存するリモートキャッシュ機能がある。
これは複数の開発者から参照可能で、CIのみならずローカルの開発におけるビルド時間の短縮にも繋がる。
リモートキャッシュの実装としてはGoogle Cloud Storageを使う方法が最も簡単である。
bazel-remoteを使うことでAWS S3やその互換実装、Azure Blob Storage、ローカルストレージを使うこともできる。
